export const LEGACY_PROCTORING_CONFIG = {
  ...
  captureCameraEvidence: false,
  captureScreenEvidence: false,
  autoSubmit: true,
  maxViolations: 2,
  heartbeatSeconds: 30,
}
...
export function normalizeProctoringConfig(examOrConfig = {}, fallback = LEGACY_PROCTORING_CONFIG) {
  ...
  return {
    ...
    requireMicrophone: source.requireMicrophone ?? fallback.requireMicrophone,
    detectVoiceActivity: source.detectVoiceActivity ?? fallback.detectVoiceActivity,
    ...
    captureCameraEvidence: source.captureCameraEvidence ?? fallback.captureCameraEvidence,
    captureScreenEvidence: source.captureScreenEvidence ?? fallback.captureScreenEvidence,
...
export const PROCTORING_SETTING_ITEMS = [
  ...
  ['requireMicrophone', 'Bắt buộc microphone', 'Kiểm tra microphone trong suốt phiên thi nhưng không ghi âm.'],
  ['detectVoiceActivity', 'Phát hiện trao đổi bằng giọng nói', 'Báo nghi vấn khi âm thanh giọng nói kéo dài; không kết luận đây là cuộc gọi.'],
  ...
  ['captureCameraEvidence', 'Ảnh camera khi vi phạm', 'Chụp một ảnh camera khi phát hiện sự kiện đáng ngờ.'],
  ['captureScreenEvidence', 'Ảnh màn hình khi vi phạm', 'Chụp một ảnh màn hình khi phát hiện sự kiện đáng ngờ.'],
