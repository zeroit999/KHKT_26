...
function getStudentDisplayName(student = {}) {
  return student.name || student.displayName || student.email?.split('@')?.[0] || 'Chờ học sinh tham gia'
}
...
function getClassCoverStyle(classItem, index = 0) {
  const cover = classItem?.coverPhotoUrl || classItem?.coverUrl || classItem?.coverPhoto || ''
  if (!cover) return getClassCover(index)
  if (cover.includes('gradient(') || cover.trim().startsWith('url(')) return cover
  return `url("${cover}")`
}
...
function Classes() {
  ...
  useEffect(() => {
    ...
    const closeMenuOnOutsideClick = (event) => {
      const isInsideMenu = event.target.closest?.('.tile-menu, .tile-menu-popover')
  ...
  useEffect(() => {
    ...
    window.history.replaceState({ ...(window.history.state || {}), classesPage: 'list' }, '')
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setSelectedClassId((currentId) => {
          if (nextClasses.some((item) => item.id === currentId)) return currentId
          return ''
        })
  ...
  useEffect(() => {
    ...
    const studentsQuery = collection(db, 'classes', selectedClassId, 'students')

    const unsubscribe = onSnapshot(studentsQuery, (snapshot) => {
      const nextStudents = sortStudentsByJoinTime(
        snapshot.docs.map((studentDoc) => ({ id: studentDoc.id, ...studentDoc.data() })),
      )
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(subjectsQuery, (snapshot) => {
      const nextSubjects = snapshot.docs.map((subjectDoc) => ({ id: subjectDoc.id, ...subjectDoc.data() }))
      setSubjects(nextSubjects)
      setSelectedSubjectId((currentId) => {
        if (nextSubjects.some((item) => item.id === currentId)) return currentId
  ...
  useEffect(() => {
    ...
    const testsQuery = query(
      collection(db, 'classes', selectedClassId, 'subjects', selectedSubjectId, 'tests'),
      orderBy('order', 'asc'),
    )

    const unsubscribe = onSnapshot(testsQuery, (snapshot) => {
      setScoreTests(snapshot.docs.map((testDoc) => ({ id: testDoc.id, ...testDoc.data() })))
    })
  ...
  useEffect(() => {
    ...
    // }
    const scoresQuery = collection(db, 'classes', selectedClassId, 'subjects', selectedSubjectId, 'scores')

    const unsubscribe = onSnapshot(scoresQuery, (snapshot) => {
      setScoreRows(snapshot.docs.map((scoreDoc) => ({ id: scoreDoc.id, ...scoreDoc.data() })))
    })
  ...
  const classToDelete = useMemo(
    () => classes.find((classItem) => classItem.id === deleteClassId) || selectedClass || null,
    [classes, deleteClassId, selectedClass],
  )

  const classToSettings = useMemo(
    () => classes.find((classItem) => classItem.id === settingsClassId) || selectedClass || null,
  ...
  const filteredStudents = useMemo(() => {
    ...
    return sortedStudents.filter((student, index) =>
      [getStudentCode(student, index), getStudentDisplayName(student), student.email, student.phone]
  ...
  const rankedScoreRows = useMemo(() => {
    ...
    const scoresByStudentId = new Map(scoreRows.map((row) => [row.studentId || row.id, row]))

    return sortedStudents
      .map((student, studentIndex) => {
        const scoreRow = scoresByStudentId.get(student.id) || {}
        const scores = scoreTests.map((test) => scoreRow.scores?.[test.id] ?? null)
      ...
      .filter((row) => {
        ...
        return [row.studentCode, getStudentDisplayName(row.student), row.student?.email]
          .some((value) => normalizeText(value).includes(keyword))
      })
      .sort((a, b) => {
        const scoreDiff = (b.average ?? -1) - (a.average ?? -1)
        if (scoreDiff !== 0) return scoreDiff
        return getTimeValue(a.student?.createdAt) - getTimeValue(b.student?.createdAt)
      })
      .map((row, index) => ({ ...row, rank: index + 1 }))
  }, [queryText, scoreRows, scoreTests, students])

  const stats = useMemo(() => {
    const averages = rankedScoreRows.map((row) => row.average).filter((average) => average !== null)

    if (!averages.length) {
      return { max: '-', min: '-', classAverage: '-', excellent: 0, good: 0, total: rankedScoreRows.length }
    }

    return {
      max: Math.max(...averages).toFixed(1),
      min: Math.min(...averages).toFixed(1),
      classAverage: (averages.reduce((sum, score) => sum + score, 0) / averages.length).toFixed(1),
  ...
  const openClassDetail = (classId) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (currentState.classesPage !== 'detail' || currentState.classId !== classId) {
  ...
  const goBackToClassList = () => {
    if (typeof window !== 'undefined' && window.history.state?.classesPage === 'detail') {
      window.history.back()
      return
    }
  ...
  const handleStudentCellChange = (index, field, value) => {
    setStudentRows((currentRows) => currentRows.map((row, rowIndex) => (
  ...
  const removeStudentRow = (index) => {
    setStudentRows((currentRows) => (
      currentRows.length === 1 ? currentRows : currentRows.filter((_, rowIndex) => rowIndex !== index)
    ))
  }
  ...
  const handleAddStudents = async (event) => {
    ...
    const invalidEmail = validRows.some((row) => !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(row.email))
    if (invalidEmail) {
      setStudentError('Email học sinh chưa đúng định dạng.')
      return
    }

    const duplicatedEmail = validRows.some((row, index) => validRows.findIndex((item) => item.email === row.email) !== index)
    if (duplicatedEmail) {
      setStudentError('Có email bị trùng trong danh sách vừa nhập.')
      return
    }

    const existingEmails = new Set(students.map((student) => normalizeText(student.email)))
    const alreadyInClass = validRows.find((row) => existingEmails.has(row.email))
    if (alreadyInClass) {
    ...
    try {
      ...
      await Promise.all(validRows.map((row, index) => addDoc(
      ...
      setStudentError(error?.message || 'Không thể thêm học sinh. Vui lòng thử lại.')
    } finally {
  ...
  const handleUpdateStudent = async (event) => {
    ...
    const duplicatedStudent = students.find((student) => student.id !== studentToEdit.id && normalizeText(student.email) === email)
    ...
    try {
      ...
      await updateDoc(doc(db, 'classes', selectedClassId, 'students', studentToEdit.id), {
      ...
      setStudentEditError(error?.message || 'Không thể cập nhật học sinh. Vui lòng thử lại.')
    } finally {
  ...
  const handleDeleteStudent = async () => {
    ...
    try {
      ...
      batch.delete(doc(db, 'classes', selectedClassId, 'students', studentToDelete.id))
      subjects.forEach((subject) => {
        batch.delete(doc(db, 'classes', selectedClassId, 'subjects', subject.id, 'scores', studentToDelete.id))
      })
      await batch.commit()
      ...
      setStudentDeleteError(error?.message || 'Không thể xóa học sinh. Vui lòng thử lại.')
    } finally {
  ...
  const openClassSettings = (classItem) => {
    const targetClass = typeof classItem === 'string'
    ...
    setSettingsForm({
      ...
      coverPhotoUrl: targetClass.coverPhotoUrl || targetClass.coverUrl || targetClass.coverPhoto || '',
    })
    setSettingsError('')
    setMenuClassId('')
  ...
  const handleSettingsImageChange = (event, fieldName) => {
    ...
    if (file.size > MAX_CLASS_IMAGE_SIZE) {
      setSettingsError('Ảnh quá lớn. Vui lòng chọn ảnh dưới 900KB để lưu được lên Firebase.')
      event.target.value = ''
      return
    }
    ...
    reader.onload = () => {
      setSettingsError('')
      setSettingsForm((current) => ({ ...current, [fieldName]: String(reader.result || '') }))
    }
    reader.onerror = () => {
  ...
  const handleSaveSettings = async (event) => {
    ...
    try {
      ...
      setSettingsError(error?.message || 'Không thể cập nhật lớp. Vui lòng thử lại.')
    } finally {
  ...
  const handleDeleteClass = async () => {
    ...
    try {
      ...
      const subjectsSnapshot = await getDocs(collection(db, 'classes', targetClassId, 'subjects'))

      await Promise.all(subjectsSnapshot.docs.map(async (subjectDoc) => {
        await deleteQuerySnapshot(await getDocs(collection(db, 'classes', targetClassId, 'subjects', subjectDoc.id, 'tests')))
        await deleteQuerySnapshot(await getDocs(collection(db, 'classes', targetClassId, 'subjects', subjectDoc.id, 'scores')))
      }))

      await deleteQuerySnapshot(await getDocs(collection(db, 'classes', targetClassId, 'students')))
      ...
      setDeleteError(error?.message || 'Không thể xóa lớp học. Vui lòng thử lại.')
    } finally {
  ...
  const createClassModal = createOpen ? (
    <div className="modal-backdrop" role="presentation" onMouseDown={closeCreateClass}>
      <form className="class-modal" onSubmit={handleCreateClass} onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-head">
          <div>
            <p>Tạo lớp mới</p>
            <h2>Thêm lớp học cho giáo viên</h2>
          </div>
          <button type="button" className="icon-btn" onClick={closeCreateClass} aria-label="Đóng">×</button>
        </div>

        <label>
          Tên lớp <span>*</span>
          <input name="name" value={classForm.name} onChange={handleClassFormChange} placeholder="Ví dụ: 10A1" autoFocus />
        </label>

        <label>
          Khối <span>*</span>
          <input name="grade" value={classForm.grade} onChange={handleClassFormChange} placeholder="Ví dụ: 10" />
        </label>

        <p className="modal-note">Môn học mặc định: <strong>{teacherSubject}</strong>. Năm học tự lưu: <strong>{schoolYear}</strong>.</p>
        {createError ? <p className="form-error">{createError}</p> : null}

        <div className="modal-actions">
          <button type="button" className="ghost-btn" onClick={closeCreateClass} disabled={creating}>Hủy</button>
          <button type="submit" className="primary-btn modal-submit" disabled={creating}>{creating ? 'ĐANG TẠO...' : 'TẠO LỚP'}</button>
        </div>
      </form>
    </div>
  ) : null
  ...
  const addStudentModal = studentOpen ? (
    <div className="modal-backdrop" role="presentation" onMouseDown={closeAddStudents}>
      <form className="class-modal student-modal" onSubmit={handleAddStudents} onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-head">
          <div>
            <p>Thêm học sinh</p>
            <h2>{selectedClass?.name || 'Lớp học'}</h2>
          </div>
          <button type="button" className="icon-btn" onClick={closeAddStudents} aria-label="Đóng">×</button>
        </div>
        ...
        <div className="student-add-table">
          ...
          {studentRows.map((row, index) => (
            <div className="student-add-row email-only-row" key={index}>
              <input type="email" value={row.email} onChange={(event) => handleStudentCellChange(index, 'email', event.target.value)} placeholder="hoc sinh@email.com" autoFocus={index === 0} />
              <button type="button" className="row-remove-btn" onClick={() => removeStudentRow(index)} disabled={studentRows.length === 1}>×</button>
            </div>
          ))}
        </div>

        <p className="modal-note">Mã HS sẽ tự tạo dạng <strong>HS001, HS002...</strong> theo thứ tự học sinh tham gia trước/sau.</p>
        <button type="button" className="ghost-btn add-row-btn" onClick={addStudentRow}>＋ Thêm email</button>
        {studentError ? <p className="form-error">{studentError}</p> : null}

        <div className="modal-actions">
          <button type="button" className="ghost-btn" onClick={closeAddStudents} disabled={addingStudents}>Hủy</button>
          <button type="submit" className="primary-btn modal-submit" disabled={addingStudents}>{addingStudents ? 'ĐANG LƯU...' : 'LƯU HỌC SINH'}</button>
        </div>
      </form>
    </div>
  ) : null
  ...
  const editStudentModal = studentEditOpen ? (
    <div className="modal-backdrop" role="presentation" onMouseDown={closeEditStudent}>
      <form className="class-modal" onSubmit={handleUpdateStudent} onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-head">
          <div>
            <p>Chỉnh sửa học sinh</p>
            <h2>{getStudentCode(studentToEdit, sortStudentsByJoinTime(students).findIndex((student) => student.id === studentToEdit?.id))}</h2>
          </div>
          <button type="button" className="icon-btn" onClick={closeEditStudent} aria-label="Đóng">×</button>
        </div>

        <label>
          Họ và tên
          <input name="name" value={studentEditForm.name} onChange={handleStudentEditChange} placeholder="Nguyễn Văn A" autoFocus />
        </label>

        <label>
          Email <span>*</span>
          <input type="email" name="email" value={studentEditForm.email} onChange={handleStudentEditChange} placeholder="hocsinh@email.com" />
        </label>

        <label>
          Số điện thoại
          <input name="phone" value={studentEditForm.phone} onChange={handleStudentEditChange} placeholder="09..." />
        </label>

        <p className="modal-note">Mã HS giữ nguyên theo thứ tự tham gia. Trạng thái được đồng bộ từ Firebase và tự đổi khi học sinh hoàn tất dữ liệu.</p>
        {studentEditError ? <p className="form-error">{studentEditError}</p> : null}

        <div className="modal-actions">
          <button type="button" className="ghost-btn" onClick={closeEditStudent} disabled={editingStudent}>Hủy</button>
          <button type="submit" className="primary-btn modal-submit" disabled={editingStudent}>{editingStudent ? 'ĐANG LƯU...' : 'LƯU THAY ĐỔI'}</button>
        </div>
      </form>
    </div>
  ) : null
  ...
  const deleteStudentModal = studentDeleteOpen ? (
    <div className="modal-backdrop" role="presentation" onMouseDown={closeDeleteStudent}>
      <div className="class-modal delete-modal" onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-head">
          <div>
            <p>Xóa học sinh</p>
            <h2>{getStudentDisplayName(studentToDelete || {})}</h2>
          </div>
          <button type="button" className="icon-btn" onClick={closeDeleteStudent} aria-label="Đóng">×</button>
        </div>

        <p className="delete-warning">Thao tác này sẽ xóa học sinh khỏi lớp và xóa điểm số liên quan của học sinh này.</p>
        <p className="modal-note">Mã HS của các học sinh khác sẽ không tự đổi sau khi xóa.</p>
        {studentDeleteError ? <p className="form-error">{studentDeleteError}</p> : null}

        <div className="modal-actions">
          <button type="button" className="ghost-btn" onClick={closeDeleteStudent} disabled={deletingStudent}>Hủy</button>
          <button type="button" className="danger-btn" onClick={handleDeleteStudent} disabled={deletingStudent}>{deletingStudent ? 'ĐANG XÓA...' : 'XÓA HỌC SINH'}</button>
        </div>
      </div>
    </div>
  ) : null
  ...
  const deleteClassModal = deleteOpen ? (
    <div className="modal-backdrop" role="presentation" onMouseDown={closeDeleteClass}>
      <div className="class-modal delete-modal" onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-head">
          <div>
            <p>Xóa lớp học</p>
          ...
          <button type="button" className="icon-btn" onClick={closeDeleteClass} aria-label="Đóng">×</button>
        </div>

        <p className="delete-warning">
          Thao tác này sẽ xóa lớp đang chọn cùng danh sách học sinh, môn học, bài kiểm tra và điểm số liên quan trên Firebase.
        </p>
        <p className="modal-note">Hành động này không thể hoàn tác.</p>
        {deleteError ? <p className="form-error">{deleteError}</p> : null}

        <div className="modal-actions">
          <button type="button" className="ghost-btn" onClick={closeDeleteClass} disabled={deletingClass}>Hủy</button>
          <button type="button" className="danger-btn" onClick={handleDeleteClass} disabled={deletingClass}>{deletingClass ? 'ĐANG XÓA...' : 'XÓA LỚP'}</button>
        </div>
      </div>
    </div>
  ) : null
  ...
  const classSettingsModal = settingsOpen ? (
    <div className="modal-backdrop" role="presentation" onMouseDown={closeClassSettings}>
      <form className="class-modal settings-modal" onSubmit={handleSaveSettings} onMouseDown={(event) => event.stopPropagation()}>
        <div className="modal-head">
          <div>
            <p>Chỉnh sửa lớp</p>
            <h2>{classToSettings?.name || 'Lớp học'}</h2>
          </div>
          <button type="button" className="icon-btn" onClick={closeClassSettings} aria-label="Đóng">×</button>
        </div>
        ...
        <section className="settings-section">
          ...
          <label>
            Tên lớp
            <input name="name" value={settingsForm.name} onChange={handleSettingsChange} placeholder="Nhập tên lớp mới" autoFocus />
          </label>
        </section>
        ...
        <section className="settings-section">
          ...
          <div className="settings-fields">
            <label>
              Ảnh đại diện lớp
              <input type="file" accept="image/*" onChange={(event) => handleSettingsImageChange(event, 'logoUrl')} />
            </label>
            <label>
              Ảnh bìa lớp
              <input type="file" accept="image/*" onChange={(event) => handleSettingsImageChange(event, 'coverPhotoUrl')} />
            </label>
            <div className="settings-preview">
              <div className="preview-cover" style={{ backgroundImage: settingsForm.coverPhotoUrl ? `url(${settingsForm.coverPhotoUrl})` : 'linear-gradient(135deg,#dbeafe,#f8fafc)' }}>
                <div className="preview-logo">{settingsForm.logoUrl ? <img src={settingsForm.logoUrl} alt="Ảnh đại diện lớp" /> : getInitial(settingsForm.name)}</div>
              </div>
              <p>Chọn ảnh trực tiếp từ thiết bị. Ảnh sẽ được xem trước trước khi lưu.</p>
            </div>
          </div>
        </section>
        ...
        <div className="modal-actions">
          <button type="button" className="ghost-btn" onClick={closeClassSettings} disabled={savingSettings}>Hủy</button>
          <button type="submit" className="primary-btn modal-submit" disabled={savingSettings}>{savingSettings ? 'ĐANG LƯU...' : 'LƯU THAY ĐỔI'}</button>
        </div>
      </form>
    </div>
  ) : null
    ...
    return (
      <main className="classes-page">
        <section className="empty-card">
          ...
          <p>Hệ thống đang lấy dữ liệu từ Firebase...</p>
        </section>
        {createClassModal}
        {addStudentModal}
        {deleteClassModal}
    ...
    return (
      <main className="classes-page class-hub-page">
        <section className="hub-top">
          <div className="hub-action-card new-class-card" role="button" tabIndex={0} onClick={openCreateClass} onKeyDown={(event) => event.key === 'Enter' && openCreateClass()}>
        ...
        <section className="class-hub">
          ...
          <section className="hub-empty-state compact-empty-state">
            ...
            <p>Bấm <strong>Lớp mới</strong> phía trên để tạo lớp đầu tiên.</p>
          </section>
        </section>
        ...
        <section className="class-hub">
          ...
          <div className="class-card-grid">
            {classes.map((classItem, index) => (
              ...
              >
                <div className="class-cover" style={{ backgroundImage: getClassCoverStyle(classItem, index) }}>
                  <button
                  ...
                  <div className="class-icon">{classItem.logoUrl || classItem.logo ? <img src={classItem.logoUrl || classItem.logo} alt="Ảnh đại diện lớp" /> : getInitial(classItem.name)}</div>
                </div>
                <div className="class-tile-body">
                  <h3>{classItem.name}</h3>
                  <p>{classItem.teacherName || classItem.subject || 'Megaedu'}</p>
                </div>
                <button
                  className="tile-menu"
                  type="button"
                  ...
                  onClick={(event) => {
                    event.stopPropagation()
                    setMenuClassId((currentId) => (currentId === classItem.id ? '' : classItem.id))
                  }}
                >
                  ...
                </button>
                {menuClassId === classItem.id ? (
                  <div className="tile-menu-popover" onClick={(event) => event.stopPropagation()}>
                    <button type="button" className="delete-menu-item" onClick={() => openDeleteClass(classItem.id)}>🗑 Xóa lớp</button>
                  </div>
                ) : null}
              </article>
            ))}
  ...
  return (
    <main className="classes-page">
      <header className="hero">
        <div>
          <h1><span>▰</span> {selectedClass?.name || 'Lớp học'}</h1>
        ...
        <div className="hero-actions">
          ...
          <button className="back-class-btn" type="button" onClick={goBackToClassList}>← DANH SÁCH</button>

          <div className="hero-stats">
            <div><span>👥 Sĩ số</span><strong>{selectedClass?.studentCount || students.length}</strong></div>
            <div><span>▣ Học kỳ</span><strong>{semester}</strong></div>
          </div>
        </div>
      </header>
      ...
      <section className="content">
        <div className="tabs">
          <button className={activeTab === 'students' ? 'active' : ''} onClick={() => setActiveTab('students')}>HỌC SINH</button>
          <button className={activeTab === 'scores' ? 'active' : ''} onClick={() => setActiveTab('scores')}>ĐIỂM SỐ</button>
        </div>

        {activeTab === 'students' ? (
          <>
            <div className="toolbar">
              <label className="search">⌕<input value={queryText} onChange={(event) => setQueryText(event.target.value)} placeholder="Tìm kiếm học sinh..." /></label>
              <button className="primary-btn" type="button" onClick={openAddStudents}>♣ THÊM HỌC SINH</button>
            </div>
              ...
              <div className="student-grid">
                {filteredStudents.map((student, index) => (
                  <article className="student-card" key={student.id}>
                    <div className="avatar">{getInitial(getStudentDisplayName(student))}</div>
                    <div className="student-info">
                      ...
                      <div className="actions"><button type="button" onClick={() => openEditStudent(student)} title="Chỉnh sửa học sinh">✎</button><button type="button" className="danger" onClick={() => openDeleteStudent(student)} title="Xóa học sinh">■</button></div>
                    </div>
                  </article>
                ))}
              </div>
          ...
          <>
            <div className="toolbar score-toolbar">
              <div className="subject-display">
                <span>Môn học</span>
                <strong>{selectedSubject?.name || selectedClass?.subject || teacherSubject}</strong>
              </div>
              <label className="search">⌕<input value={queryText} onChange={(event) => setQueryText(event.target.value)} placeholder="Tìm kiếm học sinh..." /></label>
            </div>
              ...
              <>
                <div className="table-wrap">
                  <table>
                    <thead>
                      <tr>
                        <th>Xếp hạng</th><th>Mã HS</th><th>Họ và tên</th>
                        {scoreTests.map((test, index) => <th key={test.id}>{test.name || `Bài ${index + 1}`}<br /><small>({test.code || test.id})</small></th>)}
                    ...
                    <tbody>
                      {!rankedScoreRows.length ? (
                        <tr>
                          <td className="table-empty" colSpan={4 + scoreTests.length}>Chưa có dữ liệu</td>
                        </tr>
                      ) : null}
                      {rankedScoreRows.map((row) => (
                        <tr key={row.id}>
                          ...
                          {row.scores.map((score, index) => <td key={scoreTests[index]?.id || index}>{formatScore(score)}</td>)}
                          <td className={row.average >= 8 ? 'avg good' : row.average !== null && row.average < 6 ? 'avg weak' : 'avg'}>{formatScore(row.average)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="summary-grid">
                  <StatCard label="Điểm TB cao nhất" value={stats.max} tone="green" />
                  <StatCard label="Điểm TB thấp nhất" value={stats.min} tone="red" />
                  <StatCard label="Điểm TB lớp" value={stats.classAverage} tone="blue" />
                  <StatCard label="HS Xuất sắc (≥9.0)" value={`${stats.excellent}/${stats.total}`} tone="purple" />
                  <StatCard label="HS Giỏi (≥8.0)" value={`${stats.good}/${stats.total}`} tone="green" />
                </div>
              </>
            )}
          </>
...
@media (max-width:1100px){.class-card-grid{grid-template-columns:repeat(3,minmax(190px,1fr))}.hub-top{grid-template-columns:1fr}.class-tabs{gap:22px;overflow-x:auto}.class-tabs button{white-space:nowrap}}@media (max-width:920px){.settings-danger-row{align-items:flex-start;flex-direction:column}.settings-danger-row .danger-btn{width:100%}}@media (max-width:920px){.hero{padding:24px}.hero-actions{align-items:flex-start;flex-direction:column}.student-grid{grid-template-columns:1fr 1fr}.summary-grid{grid-template-columns:1fr 1fr}.toolbar{flex-direction:column}.primary-btn{min-height:48px}.hero-stats{gap:24px}}@media (max-width:640px){.class-card-grid{grid-template-columns:1fr}.hub-top,.class-hub{padding-left:16px;padding-right:16px}.hub-action-card{padding:18px;min-height:86px}.modal-grid{grid-template-columns:1fr}.modal-actions{flex-direction:column}.ghost-btn,.modal-submit,.danger-btn{width:100%;min-height:44px}.student-grid{grid-template-columns:1fr}.hero{height:auto;align-items:flex-start;flex-direction:column}.table-wrap{overflow-x:auto}.summary-grid{grid-template-columns:1fr}}
`
