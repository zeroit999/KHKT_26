...
function getSidebarItems(isAdmin, badges = {}) {
  return [
    ...
    { id: 'watched', label: 'Đã xem', description: 'Bài đã xem hoặc đang học', icon: HistoryIcon },
    ...
    { id: 'notifications', label: 'Thông báo', description: 'Tất cả hoạt động', icon: NotificationIcon, badge: badges.notifications },
  ]
}
...
export function MobileSidebar({ open, activeItem, canCreate, isAdmin = false, onClose, onSelect, onCreate, followingAccounts = [], onOpenFollowing, onViewFollowing, badges = {} }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] xl:hidden">
      <button type="button" className="absolute inset-0 cursor-pointer bg-black/55 backdrop-blur-sm" onClick={onClose} aria-label="Đóng menu" />
      <aside className="relative h-full w-[min(90vw,340px)] overflow-y-auto rounded-r-[28px] bg-slate-50 p-3 shadow-2xl sm:p-4 dark:bg-[#111827]">
...
export function VideoSection({ title, subtitle, children, sectionRef }) {
  return <section ref={sectionRef} className="mb-9 scroll-mt-32"><div className="mb-4"><h2 className="text-xl font-black tracking-tight sm:text-2xl">{title}</h2><p className="mt-1 hidden text-sm text-slate-500 dark:text-slate-400 sm:block">{subtitle}</p></div>{children}</section>
}
export function VideoGrid({ children }) { return <div className="grid grid-cols-1 gap-x-4 gap-y-4 min-[540px]:grid-cols-2 sm:gap-y-6 lg:grid-cols-3 2xl:grid-cols-4 min-[1800px]:grid-cols-5">{children}</div> }
export function VideoCourseCard({ course, canManage, teacherProfilesById, openMenuId, onToggleMenu, onOpen, onUpdate, onDelete, onCopy, onReport, onSave, onOpenChannel, featured = false }) {
  ...
  const locked = !course?.__canBypassRestrictions && isCourseLocked(course)
  ...
  return (
    <article className={`group relative min-w-0 overflow-visible rounded-2xl sm:rounded-[22px] border bg-white shadow-sm transition-all duration-300 sm:hover:-translate-y-1 sm:hover:shadow-2xl dark:bg-[#111827] ${menuOpen ? 'z-[120]' : 'z-0'} ${typeConfig.border} ${locked ? 'opacity-70 grayscale-[0.65]' : ''}`}>
      <div className="relative aspect-video overflow-hidden">
        <button type="button" disabled={locked} onClick={() => { if (!locked) onOpen(course) }} className={`absolute inset-0 z-10 block h-full w-full ${locked ? 'cursor-not-allowed' : 'cursor-pointer'}`} aria-label={locked ? `Bài ${stripHtml(course.title)} chưa tới thời gian mở` : `Mở ${stripHtml(course.title)}`} />
      ...
      <div className="flex gap-2.5 p-3 sm:gap-3 sm:p-3.5">
        ...
        <div className="min-w-0 flex-1">
          <button type="button" disabled={locked} onClick={() => { if (!locked) onOpen(course) }} className={`block w-full max-w-[calc(100vw-150px)] truncate whitespace-nowrap sm:max-w-[240px] text-left text-[15px] font-black leading-5 text-slate-950 transition dark:text-white sm:max-w-[240px] ${locked ? 'cursor-not-allowed' : typeConfig.text}`} title={locked ? 'Bài học chưa tới thời gian mở' : stripHtml(course.title)}>{stripHtml(course.title)}</button>
...
export function FilterModal(props) {
  ...
  return (
    <ModalShell onClose={onClose} maxWidth="max-w-3xl">
      <div className="flex h-[100dvh] max-h-[100dvh] flex-col bg-white sm:rounded-3xl sm:h-auto sm:max-h-[92vh] text-slate-950 dark:bg-[#111827] dark:text-white">
...
export function CourseFormModal({ form, setForm, editingCourse, contentType = 'video', teacherClasses, participatingClasses = [], uploadingWord, uploadingVideo = false, uploadingImage = false, lessonsRef, publisherName = '', onClose, onReset, onSubmit, onWordUpload, onVideoUpload, onImageUpload }) {
  ...
  const sourceDone = isSimulation
    ? Boolean((form.simulationMode === 'embed' && String(form.simulationUrl || '').trim()) || (form.simulationMode === 'code' && String(form.simulationLanguage === 'html' ? form.simulationHtml : form.simulationCode || '').trim()))
    : isDocument
      ? Boolean(form.wordFileUrl || form.documentImageUrl || stripHtml(form.richDocument).trim())
      : Boolean((form.lessons || []).length)
  const accessDone = Boolean(form.visibility && (form.visibility === 'public' || (form.visibility === 'private' && form.className) || (form.visibility === 'class' && form.classId)))
  ...
  const gradeOptions = [{ value: '', label: 'Chọn khối' }, { value: '10', label: 'Khối 10' }, { value: '11', label: 'Khối 11' }, { value: '12', label: 'Khối 12' }]
  const participatingClassOptions = [{ value: '', label: 'Chọn lớp' }, ...(participatingClasses || []).map((item) => ({ value: String(item.id || ''), label: `${item.name || item.className || 'Lớp học'}${item.grade ? ` · Khối ${item.grade}` : ''}` }))]
  ...
  return (
    <ModalShell onClose={onClose} maxWidth="max-w-7xl">
      {resetConfirmOpen && <div className="fixed inset-0 z-[1600] grid place-items-center bg-black/70 p-4 backdrop-blur-sm" onMouseDown={(event)=>{if(event.target===event.currentTarget)setResetConfirmOpen(false)}}><div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-[#111827]"><div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-amber-100 text-3xl dark:bg-amber-500/15">↺</div><h3 className="mt-4 text-center text-xl font-black">Đặt lại toàn bộ dữ liệu?</h3><p className="mt-2 text-center text-sm leading-6 text-slate-500 dark:text-slate-400">Tên bài, nội dung, file, danh sách bài, quiz và bản nháp hiện tại sẽ bị xóa. Thao tác này không thể hoàn tác.</p><div className="mt-6 flex gap-2"><button type="button" onClick={()=>setResetConfirmOpen(false)} className="flex-1 cursor-pointer rounded-full border border-slate-300 shrink-0 px-3 py-3 text-xs font-black sm:px-5 sm:text-sm dark:border-white/20">Giữ lại dữ liệu</button><button type="button" onClick={confirmResetForm} className="flex-1 cursor-pointer rounded-full bg-amber-500 shrink-0 px-3 py-3 text-xs font-black sm:px-5 sm:text-sm text-white shadow-lg shadow-amber-500/20 hover:bg-amber-600">Đặt lại</button></div></div></div>}
      <form onSubmit={submit} className="flex h-[100dvh] max-h-[100dvh] min-h-0 flex-col overflow-hidden bg-white text-slate-950 dark:bg-[#101827] dark:text-white sm:h-[calc(96dvh-1.5rem)] sm:max-h-[calc(96dvh-1.5rem)]">
        <header className={`relative shrink-0 overflow-hidden border-b px-3 py-3 sm:px-7 sm:py-5 dark:border-white/10 ${isSimulation ? 'border-violet-100 bg-violet-50/80 dark:bg-violet-500/[0.06]' : isDocument ? 'border-emerald-100 bg-emerald-50/80 dark:bg-emerald-500/[0.06]' : 'border-blue-100 bg-blue-50/80 dark:bg-blue-500/[0.06]'}`}>
          <div className="relative flex items-start justify-between gap-2 sm:gap-5"><div className="min-w-0"><p className={`text-[10px] font-black uppercase tracking-[0.14em] sm:text-[11px] sm:tracking-[0.18em] ${eyebrowClass}`}>{editingCourse ? 'Cập nhật nội dung' : 'Tạo nội dung mới'}</p><h2 className="mt-0.5 text-lg font-black leading-tight sm:mt-1 sm:text-2xl">{editingCourse ? 'Cập nhật' : 'Đăng'} {typeLabel}</h2><p className="mt-1 hidden text-sm text-slate-500 dark:text-slate-400 sm:block">{isDocument ? 'Tạo tài liệu trực tiếp hoặc tải Word/PDF lên thư viện.' : isSimulation ? 'Tạo nội dung mô phỏng tương tác cho người học.' : 'Tạo video bài giảng và sắp xếp nội dung học tập.'}</p></div><IconButton onClick={onClose}><CloseIcon /></IconButton></div>
        </header>
        <div className="min-h-0 flex flex-1 flex-col overflow-hidden md:flex-row">
          <aside className="shrink-0 border-b border-slate-200 bg-slate-50/95 px-2 py-2 dark:border-white/10 dark:bg-[#111827]/95 md:min-h-0 md:w-[84px] md:overflow-y-auto md:border-b-0 md:border-r md:p-3 xl:w-[290px] xl:p-4">
            <div className="flex snap-x snap-mandatory gap-1.5 overflow-x-auto overscroll-x-contain pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:block md:space-y-2 md:overflow-visible md:pb-0">{tabs.map(([id,label,index]) => <button key={id} type="button" onClick={() => setTab(id)} title={label} className={`flex min-w-max shrink-0 snap-start items-center gap-2 rounded-xl px-2.5 py-2 text-[11px] font-bold sm:px-3 sm:text-sm md:w-full md:min-w-0 md:justify-center md:px-2 md:py-3 xl:justify-start xl:gap-3 xl:px-3 ${tab === id ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-white dark:text-slate-300 dark:hover:bg-white/[0.06]'}`}><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-black/10 text-[10px]">{index}</span><span className="md:hidden xl:inline">{label}</span></button>)}</div>
          ...
          <main className="min-h-0 flex-1 overscroll-contain overflow-x-hidden overflow-y-auto px-3 pb-6 pt-4 sm:p-6 xl:p-7">
            ...
            {tab === 'simulation' && isSimulation && <FormSection eyebrow="Không gian tương tác" title="Thiết kế bài mô phỏng" description="Nhúng mô phỏng có sẵn hoặc tự viết bằng HTML, Python, TypeScript và Rust."><div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={()=>{updateField('simulationMode','embed');updateField('attachMode','simulation')}} className={`rounded-2xl border p-4 text-left transition ${form.simulationMode === 'embed' ? 'border-violet-500 bg-violet-50 text-violet-700 shadow-sm dark:bg-violet-500/10 dark:text-violet-300' : 'border-slate-200 hover:border-violet-300 dark:border-white/10 dark:hover:border-violet-400/30'}`}><span className="mb-3 grid h-10 w-10 place-items-center rounded-xl bg-violet-100 text-xl dark:bg-violet-500/15">↗</span><b>Nhúng mô phỏng</b><span className="mt-1 block text-xs leading-5 text-slate-500">Dùng URL HTTPS từ PhET, GeoGebra hoặc nguồn tương tác khác.</span></button><button type="button" onClick={()=>{updateField('simulationMode','code');updateField('attachMode','simulation');if(!form.simulationLanguage)updateField('simulationLanguage','html')}} className={`rounded-2xl border p-4 text-left transition ${form.simulationMode === 'code' ? 'border-violet-500 bg-violet-50 text-violet-700 shadow-sm dark:bg-violet-500/10 dark:text-violet-300' : 'border-slate-200 hover:border-violet-300 dark:border-white/10 dark:hover:border-violet-400/30'}`}><span className="mb-3 grid h-10 w-10 place-items-center rounded-xl bg-violet-100 font-mono text-sm font-black dark:bg-violet-500/15">&lt;/&gt;</span><b>Tự viết mã mô phỏng</b><span className="mt-1 block text-xs leading-5 text-slate-500">Chọn ngôn ngữ, nhập mã và xem trước ngay trong trình duyệt.</span></button></div>{form.simulationMode === 'embed' ? <><Input label="URL mô phỏng *" value={form.simulationUrl} onChange={(v)=>updateField('simulationUrl',v)} /><SimulationPreview url={form.simulationUrl} /></> : <SimulationCodeWorkspace form={form} setForm={setForm} updateField={updateField} />}<ThumbnailPicker value={form.thumbnail} fileName={form.thumbnailFileName} uploading={uploadingImage} inputRef={thumbnailInputRef} onUrlChange={(v)=>updateField('thumbnail',v)} onUpload={(event)=>onImageUpload?.(event,'thumbnail')} /></div></FormSection>}
            ...
            {tab === 'access' && <FormSection eyebrow="Xuất bản" title="Quyền truy cập và thời gian mở" description="Bài học tự mở khi tới thời điểm được chỉ định."><div className="grid gap-4 md:grid-cols-2"><SmartFilterSelect label="Quyền truy cập" value={form.visibility} onChange={(value) => setForm((prev) => ({ ...prev, visibility: value, classId: '', className: value === 'public' ? '' : (value === 'private' && ['10','11','12'].includes(String(prev.className || '')) ? prev.className : '') }))} options={[{value:'public',label:'Công khai'},{value:'private',label:'Dành cho khối'},{value:'class',label:'Dành cho lớp'}]} icon="◉" />{form.visibility === 'private' && <SmartFilterSelect label="Khối" value={form.className} onChange={(v) => updateField('className',v)} options={gradeOptions} icon="🏫" />}{form.visibility === 'class' && <SmartFilterSelect label="Lớp học" value={form.classId || ''} onChange={(classId) => { const selectedClass = (participatingClasses || []).find((item) => String(item.id) === String(classId)); setForm((prev) => ({ ...prev, classId, className: selectedClass?.name || selectedClass?.className || '' })) }} options={participatingClassOptions} icon="🏫" />}<div className="md:col-span-2"><OpenSchedulePicker value={form.openAt || ''} onChange={(value) => updateField('openAt', value)} /></div><Input label="Mã khóa học" value={form.courseCode} onChange={() => {}} disabled /><label className="md:col-span-2 flex cursor-pointer items-start gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-400/20 dark:bg-blue-500/10"><input type="checkbox" checked={Boolean(form.publishConfirmed)} onChange={(e) => updateField('publishConfirmed',e.target.checked)} className="mt-1 h-4 w-4 accent-blue-600" /><span><b>Tôi xác nhận quyền xuất bản</b><span className="mt-1 block text-xs text-slate-500 dark:text-slate-400">Nội dung không vi phạm bản quyền và đã được kiểm tra.</span></span></label></div></FormSection>}
          </main>
        </div>
        <footer className="relative z-20 shrink-0 border-t border-slate-200 bg-white/95 px-3 pb-[calc(env(safe-area-inset-bottom)+10px)] pt-2.5 backdrop-blur dark:border-white/10 dark:bg-[#101827]/95 sm:px-5 sm:py-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><p className="hidden min-w-0 text-xs text-slate-500 lg:block">Bản nháp được tự động lưu.</p><div className="grid w-full grid-cols-2 gap-2 sm:ml-auto sm:flex sm:w-auto sm:flex-wrap sm:justify-end">{!editingCourse && <button type="button" onClick={resetForm} disabled={submitting || uploadingWord || uploadingVideo || uploadingImage} className="min-w-0 cursor-pointer rounded-xl border border-amber-300 bg-amber-50 px-3 py-2.5 text-xs font-bold sm:rounded-full sm:px-4 sm:text-sm text-amber-700 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300 sm:w-auto sm:min-w-[104px] sm:px-5">Đặt lại</button>}<button type="button" onClick={onClose} className="min-w-0 cursor-pointer rounded-xl border border-slate-300 px-3 py-2.5 text-xs font-bold sm:rounded-full sm:px-4 sm:text-sm dark:border-white/20 sm:w-auto sm:min-w-[96px] sm:px-5">Hủy</button><button type="submit" disabled={submitting || uploadingWord || uploadingVideo || uploadingImage || !canSubmit} className="col-span-2 min-w-0 rounded-xl bg-blue-600 px-5 py-3 text-sm font-black whitespace-nowrap text-white shadow-lg shadow-blue-600/20 sm:rounded-full sm:py-2.5 disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto sm:min-w-[156px] sm:px-6">{submitting ? 'Đang xử lý...' : editingCourse ? 'Lưu cập nhật' : isSimulation ? 'Gửi mô phỏng' : isDocument ? 'Gửi tài liệu' : 'Gửi video'}</button></div></div></footer>
      </form>
    </ModalShell>
  )
}
...
const simulationLanguages = [
  { id: 'html', label: 'HTML / CSS / JS', badge: 'WEB', description: 'Phù hợp giao diện và hoạt ảnh tương tác.', tone: 'from-orange-500 to-rose-500' },
  { id: 'typescript', label: 'TypeScript', badge: 'TS', description: 'Biên dịch sang JavaScript để chạy mô phỏng.', tone: 'from-blue-600 to-cyan-500' },
]

function SimulationCodeWorkspace({ form, setForm, updateField }) {
  const language = form.simulationLanguage || 'html'
  const current = simulationLanguages.find((item) => item.id === language) || simulationLanguages[0]
  const code = language === 'html' ? form.simulationHtml || '' : form.simulationCodes?.[language] || form.simulationCode || ''
  const placeholders = {
    html: '<div class="card">Mô phỏng của bạn</div>\n<style>\n.card { padding: 24px; }\n</style>\n<script>\nconsole.log("ZUNY")\n<\/script>',
    python: 'from js import document\n\nroot = document.getElementById("app")\nroot.innerHTML = "<h2>Mô phỏng Python</h2>"',
    typescript: 'const app = document.getElementById("app")!;\napp.innerHTML = `<h2>Mô phỏng TypeScript</h2>`;',
    rust: 'fn main() {\n    println!("Mô phỏng Rust trên ZUNY");\n}',
  }
  return <div className="space-y-4"><div><div className="mb-2 flex items-center justify-between gap-3"><div><p className="text-sm font-black">Ngôn ngữ mô phỏng</p><p className="mt-0.5 text-xs text-slate-500">Chọn một ngôn ngữ để mở trình soạn thảo phù hợp.</p></div><span className={`rounded-full bg-gradient-to-r ${current.tone} px-3 py-1 text-[10px] font-black text-white`}>{current.badge}</span></div><div className="grid grid-cols-2 gap-2">{simulationLanguages.map((item)=><button key={item.id} type="button" onClick={()=>updateField('simulationLanguage',item.id)} className={`group rounded-2xl border p-3 text-left transition ${language===item.id?'border-violet-500 bg-violet-50 ring-2 ring-violet-500/10 dark:bg-violet-500/10':'border-slate-200 hover:-translate-y-0.5 hover:border-violet-300 dark:border-white/10'}`}><span className={`grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br ${item.tone} text-xs font-black text-white shadow-sm`}>{item.badge}</span><span className="mt-2 block text-sm font-black">{item.label}</span><span className="mt-1 block text-[11px] leading-4 text-slate-500 dark:text-slate-400">{item.description}</span></button>)}</div></div><div className="overflow-hidden rounded-2xl border border-slate-200 bg-[#0b1020] shadow-sm dark:border-white/10"><div className="flex items-center justify-between border-b border-white/10 px-4 py-3"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-rose-400"/><span className="h-2.5 w-2.5 rounded-full bg-amber-400"/><span className="h-2.5 w-2.5 rounded-full bg-emerald-400"/><span className="ml-2 text-xs font-black text-slate-300">main.{language==='html'?'html':language==='python'?'py':language==='typescript'?'ts':'rs'}</span></div><span className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">ZUNY Simulation Lab</span></div><textarea value={code} onChange={(event)=>{const value=event.target.value;if(language==='html')updateField('simulationHtml',value);else setForm((prev)=>({...prev,simulationCode:value,simulationCodes:{...(prev.simulationCodes||{}),[language]:value}}))}} rows={16} spellCheck={false} placeholder={placeholders[language]} className="min-h-[360px] w-full resize-y bg-transparent p-4 font-mono text-[13px] leading-6 text-slate-100 outline-none placeholder:text-slate-600"/></div>{language==='rust'&&<div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800 dark:border-amber-400/20 dark:bg-amber-500/10 dark:text-amber-200"><b>Rust:</b> trình duyệt không biên dịch trực tiếp mã Rust nguồn. Hệ thống vẫn lưu mã đầy đủ; để chạy thực tế cần biên dịch sang WebAssembly.</div>}<SimulationPreview html={language==='html'?code:''} language={language} code={code}/></div>
}

function buildSimulationDocument(language, code) {
  ...
  if (language === 'rust') return `<!doctype html><html><head><meta charset="utf-8"><style>body{font-family:system-ui;margin:0;padding:28px;background:#fff7ed;color:#7c2d12}.box{border:1px solid #fdba74;border-radius:18px;padding:22px;background:white}pre{overflow:auto;background:#111827;color:#f8fafc;padding:16px;border-radius:12px}</style></head><body><div class="box"><h2>Rust → WebAssembly</h2><p>Mã Rust đã được lưu. Hãy biên dịch sang WebAssembly để chạy mô phỏng thật trong trình duyệt.</p><pre>${safeCode.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre></div></body></html>`
  return safeCode
}

function SimulationPreview({url,html,language='html',code=''}) { const srcDoc=url?'':buildSimulationDocument(language, language==='html'?html:code); return <div className="overflow-hidden rounded-2xl border border-violet-200 bg-white dark:border-violet-400/20"><div className="flex items-center justify-between border-b border-violet-100 px-4 py-2 dark:border-violet-400/20"><span className="text-xs font-black text-violet-700 dark:text-violet-300">Xem trước mô phỏng</span><span className="rounded-full bg-violet-50 px-2 py-1 text-[10px] font-black uppercase text-violet-600 dark:bg-violet-500/10 dark:text-violet-300">{url?'URL':language}</span></div>{url?<iframe src={url} title="Mô phỏng" className="h-[420px] w-full bg-white" sandbox="allow-scripts allow-same-origin allow-forms allow-popups"/>:srcDoc?<iframe srcDoc={srcDoc} title="Mô phỏng tự tạo" className="h-[420px] w-full bg-white" sandbox="allow-scripts allow-forms allow-modals"/>:<div className="grid h-[320px] place-items-center text-sm text-slate-400">Nội dung xem trước sẽ xuất hiện tại đây.</div>}</div> }
...
function OpenSchedulePicker({ value, onChange }) {
  ...
  const now=new Date()
  const hour=validSelected?String(validSelected.getHours()).padStart(2,'0'):String(now.getHours()).padStart(2,'0')
  const minute=validSelected?String(validSelected.getMinutes()).padStart(2,'0'):String(now.getMinutes()).padStart(2,'0')
