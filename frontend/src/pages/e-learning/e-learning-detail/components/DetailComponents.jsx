...
export function DescriptionBox({ course, expanded, onToggle }) {
  ...
  return <section className="mt-5 min-w-0 rounded-2xl bg-slate-100 p-4 text-sm text-slate-700 dark:bg-white/[0.08] dark:text-slate-200"><div className="flex flex-wrap gap-x-3 gap-y-1 font-bold"><span>{Number(course.views || 0).toLocaleString('vi-VN')} lượt xem</span><span>{formatFullDateTime(course.createdAt)}</span><span className="min-w-0 break-words [overflow-wrap:anywhere]">{course.courseCode || course.category || 'ZUNY'}</span></div><div ref={descriptionRef} className={`mt-3 w-full min-w-0 max-w-[900px] whitespace-normal break-words leading-6 [overflow-wrap:anywhere] [&_*]:max-w-full [&_*]:whitespace-normal [&_*]:break-words ${expanded ? '' : 'line-clamp-3'}`} dangerouslySetInnerHTML={{ __html: descriptionHtml }} />{expanded && <div className="mt-4 grid w-full max-w-[900px] gap-2 border-t border-slate-200 pt-4 text-xs dark:border-white/10 sm:grid-cols-2"><span className="min-w-0 max-h-[4.5rem] overflow-y-auto break-words pr-1 leading-6 [overflow-wrap:anywhere]"><b>Chủ đề:</b> {stripHtml(course.topic) || '---'}</span><span className="min-w-0 break-words [overflow-wrap:anywhere]"><b>Độ khó:</b> {getDifficultyLabel(course.difficulty)}</span><span className="min-w-0 break-words [overflow-wrap:anywhere]"><b>Thời lượng:</b> {formatEstimatedMinutes(course.estimatedMinutes) || course.duration || '---'}</span><span className="min-w-0 break-words [overflow-wrap:anywhere]"><b>Quyền xem:</b> {course.visibility === 'private' ? `Riêng tư${course.className ? ` • ${course.className}` : ''}` : 'Công khai'}</span></div>}{hasOverflow && <button type="button" onClick={onToggle} className="mt-2 cursor-pointer font-black text-slate-950 hover:underline dark:text-white">{expanded ? 'Thu gọn' : 'Xem thêm'}</button>}</section>
}
export function MainLearningViewer({ course, mainVideoLesson, selectedLessonIndex, realCourseId, currentUser, currentRole, lessonCount, onSkipWarning, autoPlay = false, onEnded }) {
...
export function CourseInfoCard({ course, courseTeacherProfile, ratingAverage }) {
  const rows = [
    ...
    ['Thời lượng', formatEstimatedMinutes(course.estimatedMinutes) || course.duration || '---'],
    ['Lượt xem', Number(course.views || 0).toLocaleString('vi-VN')],
    ['Đánh giá', `★ ${ratingAverage}`],
    ['Chế độ', course.visibility === 'private' ? 'Riêng tư' : 'Công khai'],
    ['Lớp được xem', course.className || 'Tất cả'],
  ]
...
export function QAPanel({ courseId, currentUser, userProfile, currentRole, courseOwnerId, focusQuestionId = '', focusReplyId = '', onOpenUser }) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      showNotice('success', 'Đã xóa bình luận', 'Nội dung đã được cập nhật theo thời gian thực.')
    } catch (error) {
  ...
  return <section className="[&_button]:cursor-pointer">
    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-sky-600 dark:text-sky-300">Trao đổi trực tiếp</p><h2 className="mt-1 text-xl font-black">Hỏi đáp bài học</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Gửi câu hỏi và theo dõi câu trả lời theo thời gian thực.</p></div><span className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-black text-sky-700 dark:bg-sky-500/10 dark:text-sky-300">{questions.length} câu hỏi</span></div>
    ...
    {deleteTarget && <div className="fixed inset-0 z-[1250] grid place-items-center bg-slate-950/65 px-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#10203a]"><h3 className="text-xl font-black">Xóa bình luận?</h3><p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">Nội dung sẽ bị xóa khỏi Firebase và biến mất realtime với mọi người.</p><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setDeleteTarget(null)} className="rounded-full px-4 py-2 text-sm font-black">Hủy</button><button type="button" onClick={confirmDelete} className="rounded-full bg-rose-600 px-5 py-2 text-sm font-black text-white">Xóa</button></div></div></div>}
